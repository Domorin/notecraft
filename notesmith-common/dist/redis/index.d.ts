...
type ServiceRPCs<S extends Service> = {
    [Thing in keyof RPCs[S]]: (args: RPCs[S][Thing] extends {
        input: Record<string, unknown>;
    } ? RPCs[S][Thing]["input"] : never) => RPCs[S][Thing] extends {
        output: Record<string, unknown>;
    } ? Promise<RPCs[S][Thing]["output"]> : never;
};
...
export declare const pendingMessages: Record<number, {
...
export default function initRedis<T extends Service>(context: {
    ...
    pubsub: {
        publish: <Channel extends keyof RedisChannelType>(channel: Channel, message: RedisChannelType[Channel]) => Promise<void>;
        subscribe: <Channel_1 extends keyof RedisChannelType>(channel: Channel_1, callback: (message: RedisChannelType[Channel_1]) => void) => Promise<void>;
    };
    rpc: <Target extends Exclude<"App", T> | Exclude<"Ws", T>, RPC extends keyof RPCs[Target] & string, IgnoreResponse extends boolean = false>(target: Target, rpc: RPC, input: Parameters<ServiceRPCs<Target>[RPC]>[0], opts?: {
        ignoreResponse: IgnoreResponse;
    }) => Promise<IgnoreResponse extends false ? ReturnType<ServiceRPCs<Target>[RPC]> : undefined>;
};
export {};
