...
import { applyAwarenessUpdate, removeAwarenessStates, encodeAwarenessUpdate, CustomAwareness, } from "./custom_awareness";
...
messageHandlers[messageQueryAwareness] = (encoder, _decoder, provider, _emitSynced, _messageType) => {
    ...
    encoding.writeVarUint8Array(encoder, encodeAwarenessUpdate(provider.awareness, Array.from(provider.awareness.getStates().keys())));
};
...
function tryParseCustomMessage(data) {
    try {
        const message = SuperJSON.parse(data);
    ...
    if (provider.shouldConnect && provider.ws === null) {
        ...
        websocket.onopen = () => {
            ...
            if (provider.awareness.getLocalState() !== null) {
                ...
                encoding.writeVarUint8Array(encoderAwarenessState, encodeAwarenessUpdate(provider.awareness, [
...
export class CustomProvider extends Observable {
     ...
     */
    constructor(serverUrl, roomname, doc, { connect = true, awareness = new CustomAwareness(doc), params = {}, WebSocketPolyfill = WebSocket, resyncInterval = -1, maxBackoffTime = 2500, disableBc = false, customMessageHandler = undefined, } = {}) {
        super();
    ...
    connectBc() {
        ...
        const encoderAwarenessState = encoding.createEncoder();
        encoding.writeVarUint(encoderAwarenessState, messageAwareness);
        encoding.writeVarUint8Array(encoderAwarenessState, encodeAwarenessUpdate(this.awareness, [this.doc.clientID]));
    ...
    disconnectBc() {
        ...
        const encoder = encoding.createEncoder();
        encoding.writeVarUint(encoder, messageAwareness);
        encoding.writeVarUint8Array(encoder, encodeAwarenessUpdate(this.awareness, [this.doc.clientID], new Map()));
        broadcastMessage(this, encoding.toUint8Array(encoder));
        if (this.bcconnected) {
